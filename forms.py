...
class InputForm(FlaskForm):
    ...
    duration = IntegerField(
        # Removed label since it’s hidden
        render_kw={"type": "hidden"},  # Make it a hidden input
        validators=[DataRequired()]  # Still required for form validation
    )
    total_stops = IntegerField(
        label="Total Stops",
        validators=[DataRequired()]
    )
    no_of_passengers = IntegerField(
