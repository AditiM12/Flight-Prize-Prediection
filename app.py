...
def predict():
    ...
    if request.method == "POST":
        if form.validate_on_submit():
            ...
            if model:
                try:
                    predicted_price_per_person = model.predict(x_new)[0]
                    total_price = predicted_price_per_person * no_of_passengers
                    duration_hours = form.duration.data / 60
                    message = f"The predicted price per passenger is {predicted_price_per_person:,.0f} INR. "
                    message += f"Duration: {duration_hours:.2f} hours. "
            ...
            flash("Form validation failed. Please check all fields, including Departure and Arrival Times.", "danger")
